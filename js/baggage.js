...
function handleEditBaggage(e) {
    ...
    fetch(`php/api/baggage/read.php?id=${baggageId}`)
        ...
        .then(baggage => {
            ...
            document.getElementById('weight').value = (baggage.Weight !== null && baggage.Weight !== undefined && baggage.Weight !== '') ? baggage.Weight : 0;
...
function handleBaggageSubmit(e) {
    ...
    // Build JSON object with correct keys and valid number for weight
    const data = {
        'booking-id': formData.get('booking-id'),
        'weight': formData.get('weight') ? parseFloat(formData.get('weight')) : 0,
        'baggage-type': formData.get('baggage-type'),
